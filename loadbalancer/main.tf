...
resource "azurerm_network_interface_backend_address_pool_association" "association1" {
  depends_on              = [azurerm_lb.lb,azurerm_lb_backend_address_pool.backend_pool]
  for_each                = var.loadbal
...
resource "azurerm_lb_probe" "lb_probe1" {
  depends_on      = [azurerm_lb.lb,azurerm_network_interface_backend_address_pool_association.association1, azurerm_network_interface_backend_address_pool_association.association2, azurerm_lb_backend_address_pool.backend_pool]
  for_each        = var.loadbal
  loadbalancer_id = azurerm_lb.lb[each.key].id
